module.exports = (app) => {
  ...
  const save = async (req, res) => {
    ...
    if (reservation.id) {
      ...
      app.db
        .raw("SELECT sp_make_reservation FROM sp_make_reservation(?,?,?);", [
  ...
  const get = async (req, res) => {
    ...
    const result = await app.db(key).count("codigo").first();
  ...
  const filterByDate = async (req, res) => {
    app.db
      .raw(
        "SELECT type_of_room, description, image_url from sp_filter_room(?, ?);",
        [req.query.in, req.query.out]
      )
